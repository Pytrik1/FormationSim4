...
class controller:
    ...
    def controller(self, data):
        ...
        if self.running < 10:
            ...
            mu_hat_dot = 2*(np.array([self.Ed[1]- self.mu_hat[0], self.Ed[2]- self.mu_hat[1]]))
            self.mu_hat = self.mu_hat + mu_hat_dot * self.DT
            self.S1bDz = np.array([[self.z_values[4]/self.z_values[3], self.z_values[7]/self.z_values[6]], \
                                   [self.z_values[5]/self.z_values[3], self.z_values[8]/self.z_values[6]]])
