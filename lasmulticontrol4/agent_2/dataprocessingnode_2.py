...
class determine_z_values:
            ...
            if self.k == 2: # tracking state
                ...
                for r in range(len(self.robots)):
                    ...
                    if a >= 360:
                        a = a - 360
                    self.robots[r] = (d,a)
                    # print 'distance,angle' , self.robots[1]
                ...
                self.zx_1 = np.float32(np.cos((self.robots[0][1]-np.int_(180))*2*np.pi/360)*self.robots[0][0]) # edge 1
                self.zy_1 = np.float32(np.sin((self.robots[0][1]-np.int_(180))*2*np.pi/360)*self.robots[0][0]) # edge 1
                self.zx_2 = np.float32(np.cos((self.robots[1][1]-np.int_(180))*2*np.pi/360)*self.robots[1][0]) # edge 3 
                self.zy_2 = np.float32(np.sin((self.robots[1][1]-np.int_(180))*2*np.pi/360)*self.robots[1][0]) # edge 3
                self.zx_3 = np.float32(np.cos((self.robots[2][1]-np.int_(180))*2*np.pi/360)*self.robots[2][0]) # edge 2 
                self.zy_3 = np.float32(np.sin((self.robots[2][1]-np.int_(180))*2*np.pi/360)*self.robots[2][0]) # edge 2

                # self.zx_1 = np.float32(np.cos((self.robots[0][1]-np.int_(180))*2*np.pi/360)*self.robots[0][0]) # edge 1 
                # self.zy_1 = np.float32(np.sin((self.robots[0][1]-np.int_(180))*2*np.pi/360)*self.robots[0][0]) # edge 1
                # self.zx_2 = np.float32(np.cos((self.robots[2][1]-np.int_(180))*2*np.pi/360)*self.robots[2][0]) # edge 3 
                # self.zy_2 = np.float32(np.sin((self.robots[2][1]-np.int_(180))*2*np.pi/360)*self.robots[2][0]) # edge 3
                # self.zx_3 = np.float32(np.cos((self.robots[1][1]-np.int_(180))*2*np.pi/360)*self.robots[1][0]) # edge 2 
                # self.zy_3 = np.float32(np.sin((self.robots[1][1]-np.int_(180))*2*np.pi/360)*self.robots[1][0]) # edge 2
                ...
                print self.z_values
                print self.name, '(volgorde = 1, 4, 3)'
                print '------------------------------------------------'
                print 'dist to Nexus 1 = ', round(self.z_values[0],2), self.robots[0][1]
                print 'dist to Nexus 4 = ', round(self.z_values[3],2), self.robots[1][1]
                print 'dist to Nexus 3 = ', round(self.z_values[6],2), self.robots[2][1]
                print '------------------------------------------------'
    ...
    def shutdown(self):
        rospy.loginfo("Stopping dataprocessingnode_2...")
        self.running = False
...
if __name__ == '__main__':
    rospy.init_node('dataprocessingnode_2', anonymous=False)
    determine_z_values()
    rospy.spin() 
