...
class determine_r_values:
    ...
    def __init__(self):
        ...
        rospy.Subscriber('/n_2hokuyo_points', LaserScan, self.calculate_r)
        ...
        if self.running:
            ...
            if self.k == 2: # tracking state
                ...
                def obstacle_search():
                    ...
                    until = i
                    print 'i', i
                    self.find_positions(accdata2, obstacles, until, i)
                ...
                if self.obstacles != []:
                    oxy = []
                    obstacle_dist_x_y = np.empty([3,len(self.obstacles)])
                    for idx, val in enumerate(self.obstacles):
                        oxy.append(self.calculate_x_and_y_dist(self.obstacles, idx))
                        obstacle_dist_x_y = np.array([self.obstacles[idx][0], oxy[idx][0], oxy[idx][1]],
                                                      dtype=np.float32)
                ...
                print '------------------------------------------------'
                print 'dist to Nexus 1 = ', round(self.r_values[0], 2), self.robots[0][1]
                print 'dist to Nexus 4 = ', round(self.r_values[3], 2), self.robots[1][1]
                print 'dist to Nexus 3 = ', round(self.r_values[6], 2), self.robots[2][1]
                print '------------------------------------------------'
                ...
                accdata = self.clean_data(scandata)
                accdata2 = self.clean_data(scandata2)
                print accdata2
                for i, v in enumerate(scandata2):
                    if ((v == 0) and  (scandata2[i-1] == 0) and
