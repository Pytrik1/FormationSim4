...
class controller:
    ...
    def __init__(self):
        ...
        # prepare publisher
        self.pub = rospy.Publisher('/n_3/cmd_vel', Twist, queue_size=10)
        # self.pub = rospy.Publisher('/nexus3/cmd_vel', Twist)
        self.velocity = Twist()
    ...
    def controller(self, data):
        ...
        if self.running < 10:
            ...
            p23 = np.array([[z_values[1]], [z_values[2]]])
            p13 = np.array([[z_values[4]], [z_values[5]]])
            p43 = np.array([[z_values[7]], [z_values[8]]])

            # Formation shape control
            BbDz = np.array([[z_values[1], z_values[7]], \
                                  [z_values[2], z_values[8]]])
            Dzt = np.array([[(z_values[0])**(-1), 0], \
                                 [0, (z_values[6])**(-1)]])
