...
class determine_z_values:
    ...
    def calculate_z(self, msg):
        ...
        print 'running3'
        scandata= np.asarray(msg.ranges)+0.03# - 0.01
        ...
        if self.running:
            ...
            if self.k == 2: # tracking state
                ...
                self.zx_1 = np.float32(np.cos((self.robots[0][1]-np.int_(180))*2*np.pi/360)*self.robots[0][0]) # edge 2
                self.zy_1 = np.float32(np.sin((self.robots[0][1]-np.int_(180))*2*np.pi/360)*self.robots[0][0]) # edge 2
                ...
                self.zx_3 = np.float32(np.cos((self.robots[2][1]-np.int_(180))*2*np.pi/360)*self.robots[2][0]) # edge 5
                self.zy_3 = np.float32(np.sin((self.robots[2][1]-np.int_(180))*2*np.pi/360)*self.robots[2][0]) # edge 5
                ...
                self.z_values = np.array([self.robots[2][0], self.zx_3, self.zy_3, \
                                          self.robots[0][0], self.zx_1, self.zy_1, \
                                          self.robots[1][0], self.zx_2, self.zy_2], dtype=np.float32)
                print self.name, '(volgorde = 2, 1, 4)'
                print '------------------------------------------------'
                print 'dist to Nexus 2 = ', round(self.z_values[0],2),self.robots[0][1]
                print 'dist to Nexus 1 = ', round(self.z_values[3],2),self.robots[1][1]
                print 'dist to Nexus 4 = ', round(self.z_values[6],2),self.robots[2][1]
                print '------------------------------------------------'
                print self.z_values
                if ok:
...
# Change for sim:
###        rospy.Subscriber('/nexus3/scan', numpy_msg(Floats), self.calculate_z)
