...
class determine_z_values:
    ...
    def calculate_z(self, msg):
		...
		for i in range(len(z_a)-1):  # Calculates the number of robots
                ...
                if z_a[-1]==359 and z_a[-2]!=358:
                    n=n-1
                    z_a[-1]=0

                if z_a[0]==0 and z_a[1]!=1:
                    n=n-1
                    z_a[0]=z_a[1]-1
                
                if z_a[0]==0 and z_a[-1]==359:
                    n=n-1
		...
		for i in range(P,len(z_a)-1):
                ...
                if z_a[0]==0 and z_a[1]==1 and z_a[-1]==359:  #This loop makes sure that a robot is not read twice if it is located at the scan starting point
                    R[0]= np.append(R[0],R[-1])
                    del R[-1] 
		    #n=n-1
               # if 0 in R[0] or 1 in R[0] and 358 in R[-1] or 359 in R[-1]
               #       R[0]= np.append(R[0],R[-1])
...
###                self.da = self.da - 2
    ...
    def shutdown(self):
        ...
        self.pub.publish(self.z_values)
        print ('Shutting Down Data Processor 4')
	rospy.sleep(1)
