...
class controller:
    ...
    def controller(self, data):
	    ...
	    if isinstance(self.mu_hat,float):
	        if z_values[1]<0 and z_values[2]<0 and z_values[-1]>0 and z_values[-2]<0:
	        ...
	        if z_values[1]<0 and z_values[2]<0 and z_values[-1]>0 and z_values[-2]<0 and z_values [-4]>0:                
	            mu_hat_dot = 2*(np.array([Ed[-1]- self.mu_hat[0], Ed[-2]- self.mu_hat[1]]))
	            self.mu_hat = self.mu_hat + mu_hat_dot * self.DT
		    self.S1bDz = np.array([[z_values[-5]/z_values[-6], z_values[-4]/z_values[-6]], \
		                           [z_values[-2]/z_values[-3], z_values[-1]/z_values[-3]]])
		    print "estimating last and penultimate agents (Multiple)"
		else:
		    mu_hat_dot = 2*(np.array([Ed[1]- self.mu_hat[0], Ed[-1]- self.mu_hat[1]]))
		    self.mu_hat = self.mu_hat + mu_hat_dot * self.DT
		    self.S1bDz = np.array([[z_values[1]/z_values[0], z_values[2]/z_values[0]], \
		                           [z_values[-2]/z_values[-3], z_values[-1]/z_values[-3]]])
		    print "estimating first and last agents (Multiple)"
...
#            self.z = np.array([self.z_values[7], self.z_values[8], self.z_values[1], self.z_values[2]])
		...
		U = self.c*BbDz.dot(Dzt).dot(Ed) + 0.5*self.S1bDz.dot(self.mu_hat)
