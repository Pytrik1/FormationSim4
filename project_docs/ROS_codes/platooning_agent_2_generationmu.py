...
class process_laservalues:
	...
	def error(self, store_x_distance, store_y_distance, store_angle,length):
		...
		if length == 0: # no data present
			...
			self.merging_agent = 0
				...
				#print 'e_x_pos_vec', self.name, e_x_pos_vec
				#print 'e_x_neg_vec', self.name, e_x_neg_vec
				#print 'e_y_vec', self.name, e_y_vec
	...
	def generation_mu(self, e_y,e_x_pos,e_x_neg):
		...
		for i in range(1,n):
				...
				if i == n-1:
					...
					print 'mu_log', self.mu_log
					print 'mu_hat_log', self.mu_hat_log
					print 'self.sum_distance_d_t', self.sum_distance_d_t
	...
	def shutdown(self):
		...
		with open('/home/s2504111/monte_carlo_simulations/output/withmu/AAA_convergence_mu_agent_2.csv', 'a') as new_file: #CHANGE W+ TO A (APPEND)
...
if __name__ == '__main__':
	rospy.init_node('platooning_agent_n_2_generationmu', anonymous=False) 
	speed = 4
